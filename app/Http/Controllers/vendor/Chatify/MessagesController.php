...
class MessagesController extends Controller
{
    ...
    {
        $user = auth()->user();
        $restrictedRoles = [User::AGENT_ROLE];

        if (in_array($user->role_id, $restrictedRoles)) {
            return abort(403, "Sorry, You are not allowed to download media.");
        }
    ...
    public function search(Request $request)
    {
        ...
        $eligibleRoles = $this->getChatEligibleRoles($currentUser->role_id);

        $records = User::where('id', '!=', $currentUser->id)
            ->where(function ($query) use ($eligibleRoles, $currentUser) {
                if (count($eligibleRoles)) {
                    $query->whereIn('role_id', $eligibleRoles);
                    // } else {
                    //     // Superadmin case or similar, allow all
                    //     $query->where('role', '!=', $currentUser->role_id);
                }
            })
    ...
    private function getChatEligibleRoles(int $userRole): array
    {
        switch ($userRole) {
            case User::SUPERADMIN_ROLE:
                return [];
            case User::MANAGER_ROLE:
                return [User::SUPERADMIN_ROLE, User::AGENT_ROLE];
            case User::AGENT_ROLE:
                return [User::MANAGER_ROLE];
            case User::RND_ADMIN_ROLE:
                return [User::SUPERADMIN_ROLE, User::RND_ROLE];
            case User::RND_ROLE:
                return [User::RND_ADMIN_ROLE];
            default:
                return [0];
        }
    }
